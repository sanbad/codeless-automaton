...
class DropDownSearchApi extends React.Component {
    ...
    handleChange = (e) => {
        ...
        let items = this.props.elementsList.filter(item => item.label.toLowerCase().includes(value.toLowerCase()));
        this.setState({ 
            [name]: value,
            searchArray:items
        });
        ...
        return (
            <DropdownWrapper>
                <Dropdown  onToggle={this.toggleMethod}>
                    <Dropdown.Toggle>
                        <Placeholder fontSize={"13px"}>{this.props.placeholder}</Placeholder>
                        <DownArrow />
                    </Dropdown.Toggle>
                    <Dropdown.Menu>
                        <SearchInputLayout>
                        ...
                        <DropdownScrollable>
                        {
                            dropdownArray && dropdownArray.map((item)=>{
                                return <Dropdown.Item onClick={this.onClickListItem.bind(this,item)}>
                                    <DropdownText fontSize={"12px"}>{item.label}</DropdownText>
                                </Dropdown.Item>
                            })
                        }
                        </DropdownScrollable>
